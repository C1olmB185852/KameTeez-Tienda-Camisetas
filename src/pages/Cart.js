...
const Cart = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 flex flex-col lg:flex-row lg:gap-10">
      {/* Lista de productos */}
      <div className="lg:w-2/3 w-full bg-white p-6 rounded-lg shadow-lg border border-gray-200">
        ...
        {cart.length > 0 ? (
          cart.map((item) => (
            <div
              key={item.id}
              className="flex flex-col md:flex-row items-center mb-6 p-4 bg-gray-50 rounded-lg shadow-sm hover:shadow-xl transition-all"
            >
              <button
      ...
      {/* Totales del carrito */}
      <div className="lg:w-1/3 w-full bg-gray-100 p-6 rounded-lg shadow-lg mt-10 lg:mt-0 border border-gray-200">
        <h2 className="text-2xl md:text-3xl font-semibold mb-6 text-gray-800">
          Totales del Carrito
        </h2>
